...
function RouletteResultBreakdown({
  nonce,
  serverSeed,
  clientSeed,
}: RouletteResultBreakdownProps): JSX.Element {
  ...
  const finalOutcome = Math.floor(outcome * 37);
      ...
      <div>
        ...
        <div className="flex gap-2 text-xs overflow-x-auto no-scrollbar">
          <div className="grid grid-cols-11">
            <div className="col-span-11 font-semibold tracking-widest">{`(${selectedBytes.join(', ')}) -> [0, ..., 36] = ${finalOutcome}`}</div>
            ...
            <span className="font-semibold col-span-5 place-self-start tracking-widest">
              {String((outcome * 37).toFixed(12)).split('.')[0]}
              <span className="text-neutral-weak">
                .{String((outcome * 37).toFixed(12)).split('.')[1]}
              </span>
            </span>
          </div>
        </div>
...
export default RouletteResultBreakdown;
